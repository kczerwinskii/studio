...
## 2026-09-25, Claude (Fable): autor rolki przez oEmbed zamiast ukrytego okna

Próba "fitness polska" (PL, 100 tys.): 147 kandydatów, tylko 2 obce, 23 zapytania, ale 4 twórców w 10 minut. Czas szedł w ukryte okno Electrona, które dla każdej rolki ładuje pełną stronę Instagrama (do 22 s, sekwencyjnie), bo lista hashtagu z API nie podaje autora. Nowa funkcja `autorZOembed` w `research-meta.js`: oficjalny endpoint `/instagram_oembed` z polem `author_name`, jedno szybkie zapytanie. Pula (`research-pula.js`) próbuje oEmbed najpierw; przy braku dostępu (kody 10/100/200/3) wyłącza go na resztę partii, pokazuje komunikat "Dodaj funkcję oEmbed Read w aplikacji Meta" i wraca do odczytu strony. oEmbed Read trzeba dodać w panelu aplikacji Meta tak samo jak Public Content Access. Testy: `test_research_pula.js` (autor z oEmbed bez odczytu strony, jedna próba i zapas przy braku dostępu).

## 2026-09-25, Claude (Fable): Research wyłączony w interfejsie

Decyzja Kuby po dniu prób. Podsumowanie uczciwe: oficjalne API Mety nie ma wyszukiwania po słowie (tylko hashtagi), nie podaje wyświetleń ani autora w liście, widzi tylko konta firmowe i twórcze, a limit to około 200 zapytań na godzinę. Do celu "wpisuję temat, widzę hity z milionami" wyszukiwarka w aplikacji Instagram jest szybsza i zawsze będzie. Wartość, która została i może wrócić: wybicia u obserwowanych twórców względem ich własnej mediany (Business Discovery znanych kont, 1 zapytanie na konto) oraz Analiza własnych rolek.

Zmiana: usunięty przycisk zakładki i skrypt `research.js` z `app/index.html`, wywołanie `Research.start()` z `app/app.js`, zapamiętany widok "research" otwiera Analizę. Moduły `moduly/research*.js`, panel `app/research*.js`, testy i dokumenty zostają w repozytorium bez zmian. Trasy `/api/research/...` nadal istnieją, ale bez panelu nikt ich nie odpytuje, więc zaplanowane wznowienie kolejki nie zużyje limitu Meta. Przywrócenie: cofnąć ten commit.
