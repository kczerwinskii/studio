  ...
  for(const tryb of ["top_media","recent_media"]){
    try {
      ...
      for(let strona=0;strona<(tryb==="top_media"?3:1);strona++){
        const d=await graph(n,"/"+id+"/"+tryb,{user_id:ust.ig_id,fields:"id,caption,media_type,permalink,timestamp,like_count,comments_count",limit:10,...(after?{after}:{})});
...
async function pobierzStrone(n,fraza,poprzednia={}){
  ...
  const tor=stan[tryb],d=await graph(n,"/"+stan.id+"/"+tryb,{user_id:ust.ig_id,fields:"id,caption,media_type,permalink,timestamp,like_count,comments_count",limit:10,...(tor.after?{after:tor.after}:{})});
...
async function pobierzProfil(n,username,opcje={}){
  ...
  const posty=new Map(),kursory=new Set();let after,koniec=false,ostrzezenie=null;
  const strony=opcje.strony||1;
  ...
  return {posty:[...posty.values()],koniec,ostrzezenie,url:"https://www.instagram.com/"+username+"/reels/",zrodlo_api:"meta"};
}
