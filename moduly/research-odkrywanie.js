...
async function obsluzOdkrywanie(req,res,url,n) {
  ...
  try {
    ...
    if(trasa==="/szukaj") {
      ...
      const cel=Number(c.cel??30),filtry={jezyk:c.filtry?.jezyk??"pl",okres:Number(c.filtry?.okres??30),prog:Number(c.filtry?.prog??3),min_wyswietlen:Number(c.filtry?.min_wyswietlen??50000)};
      if(![30,60,100].includes(cel)||!["pl","en"].includes(filtry.jezyk)||![0,7,30].includes(filtry.okres)||![0,3,4,5].includes(filtry.prog)||![0,10000,25000,50000,100000,250000,500000,1000000].includes(filtry.min_wyswietlen))return wyslij(400,{blad:"Wybierz cel 30, 60 lub 100 i dostępne filtry."});
